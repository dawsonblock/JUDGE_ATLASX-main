"""Isolated proof test for evidence verification standard.

This test does NOT load backend/app/tests/conftest.py, so it can run
without SQLAlchemy or a database.  It imports the verification module
directly via sys.path manipulation.
"""
...
# Add backend/ to path so ``app.evidence`` is importable without pulling in
